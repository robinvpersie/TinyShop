...
- (void)touchAction:(UIButton *)button {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"changePage" object:@(self.currentPage)];
...
//    _scrollView.contentOffset = CGPointMake(APPScreenWidth * currentPage, 0);
//    lineBottom.frame = CGRectMake(_scrollView.contentOffset.x / arrayCount + x, PageBtn - 2, width, 1);
//}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.tag == 318) {
        ...
        if (arrayCount > 6) {
            ...
            lineBottom.frame = CGRectMake(scrollView.contentOffset.x / arrayCount + x, PageBtn - 2, width, 1);
        }
...
- (void)initUI {
    ...
    NSString *title = titlesArray[1];
    
    width = [UILabel getWidthWithTitle:title font:[UIFont systemFontOfSize:17]];
    
    x = APPScreenWidth/titlesArray.count/2 - width/2;
    
//    lineBottom.frame = CGRectMake(0, PageBtn - 2,yourCount * FUll_VIEW_WIDTH, 2);
    lineBottom.frame = CGRectMake(x, PageBtn - 2,width, 2);
    topTabBottomLine.frame = CGRectMake(0, PageBtn - 1, (1 + additionCount) * FUll_VIEW_WIDTH, 1);
    
    _scrollView.contentOffset = CGPointMake(APPScreenWidth * _currentPage, 0);
    lineBottom.frame = CGRectMake(_scrollView.contentOffset.x / arrayCount + x, PageBtn - 2, width, 1);
}
