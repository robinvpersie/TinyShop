...
- (id)init {
    if (self = [super init]) {
        [self commonInit];
//        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getNotification:) name:@"segmentTitles" object:nil];
    }
    return self;
}
...
- (void)getNotification:(NSNotification *)notification {
    ...
    [titles addObjectsFromArray:object.mutableCopy];
}
...
//    LGLayer.backgroundColor = LG_BannerColor.CGColor;
    LGLayer.backgroundColor = GreenColor.CGColor;
    LGLayer.frame = CGRectMake(firstX, self.frame.size.height - 6, 55, 2);
    // 设定它的frame
