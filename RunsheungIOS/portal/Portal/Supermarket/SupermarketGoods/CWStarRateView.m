...
@property (nonatomic, assign) BOOL isFaceView;
...
- (instancetype)initWithFrame:(CGRect)frame numberOfFaces:(NSInteger)numberOfFaces {
    if (self = [super initWithFrame:frame]) {
        _numberOfStars = numberOfFaces;
        self.isFaceView = YES;
        [self buildDataAndUI];
    }
    return self;
...
- (void)buildDataAndUI {
    ...
    if (_isFaceView) {
        self.foregroundStarView = [self createStarViewWithImage:@"icon_good"];
        self.backgroundStarView = [self createStarViewWithImage:@"icon_bad"];
    } else {
