...
#define PYSearchHistoriesPath2 [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"PYRexentSearchs2.plist"]

#define PYSearchHistoriesPath3 [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"PYRexentSearchs3.plist"]
...
- (void)setSearchType:(PYSearchType)searchType {
    _searchType = searchType;
    if (_searchHistories != nil) {
        [_searchHistories removeAllObjects];
        if (_searchType == PYSearchTypeSupermarket) {
            _searchHistories = [NSKeyedUnarchiver unarchiveObjectWithFile:PYSearchHistoriesPath];
        } else if (_searchType == PYSearchTypeCanteen) {
            _searchHistories = [NSKeyedUnarchiver unarchiveObjectWithFile:PYSearchHistoriesPath2];
            NSLog(@"%@",_searchHistories);
        }else {
            _searchHistories = [NSKeyedUnarchiver unarchiveObjectWithFile:PYSearchHistoriesPath3];
        }
    }
}
...
- (void)setupSearchHistoryTags
{
    ...
    // 添加和布局标签
    self.searchHistoryTags = [self addAndLayoutTagsWithTagsContentView:self.searchHistoryTagsContentView tagTexts:[_searchHistories copy]];
}
...
- (void)emptySearchHistoryDidClick
{
    ...
    if (_searchType == PYSearchTypeSupermarket) {
        ...
        [NSKeyedArchiver archiveRootObject:self.searchHistories toFile:PYSearchHistoriesPath2];
    }else {
        [NSKeyedArchiver archiveRootObject:self.searchHistories toFile:PYSearchHistoriesPath3];
   }
