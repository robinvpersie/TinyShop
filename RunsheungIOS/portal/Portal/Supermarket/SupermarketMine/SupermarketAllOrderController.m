//
//  SupermarketAllOrderController.m
//  Portal
//
...
@interface SupermarketAllOrderController ()
...
@implementation SupermarketAllOrderController
...
- (void)requestData {
    ...
    if (self.controllerType == ControllerTypeDepartmentStores) {
        ...
        [KLHttpTool getOrderListWithStatus:0 pageIndex:_pageIndex appType:8 success:^(id response) {
            NSLog(@"%@",response);
        ...
        [KLHttpTool getOrderListWithStatus:0 pageIndex:_pageIndex appType:6 success:^(id response) {
            NSLog(@"%@",response);
            NSNumber *status = response[@"status"];
            if (status.integerValue == 1) {
                NSArray *data = response[@"data"];
