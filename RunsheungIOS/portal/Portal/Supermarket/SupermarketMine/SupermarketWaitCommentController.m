//
//  SupermarketWaitCommentController.m
//  Portal
//
...
@interface SupermarketWaitCommentController ()
...
@implementation SupermarketWaitCommentController
...
- (void)requestData {
    ...
    if (self.controllerType == ControllerTypeDepartmentStores) {
        ...
        [KLHttpTool getOrderListWithStatus:5 pageIndex:_pageIndex appType:8 success:^(id response) {
            NSLog(@"%@",response);
        ...
        [KLHttpTool getOrderListWithStatus:5 pageIndex:_pageIndex appType:6 success:^(id response) {
            NSLog(@"%@",response);
            NSNumber *status = response[@"status"];
            if (status.integerValue == 1) {
                NSArray *data = response[@"data"];
