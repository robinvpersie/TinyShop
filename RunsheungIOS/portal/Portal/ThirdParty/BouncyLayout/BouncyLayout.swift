...
//  Portal
//
//  Created by PENG LIN on 2017/8/4.
...
public class BouncyLayout:UICollectionViewFlowLayout {
    ...
    public override func prepare() {
        ...
        guard let view = collectionView,
              let attributes = super.layoutAttributesForElements(in: view.bounds)?.flatMap({ $0.copy() as? UICollectionViewLayoutAttributes })  else {
             return
        }
        
        oldBehaviors(for: attributes).forEach({ animator.removeBehavior($0) })
        newBehaviors(for: attributes).forEach({ animator.addBehavior($0, damping: damping, frequency: frequency) })
    ...
    private func newBehaviors(for attributes:[UICollectionViewLayoutAttributes]) -> [UIAttachmentBehavior] {
        ...
        let indexPaths = animator.behaviors.flatMap({
             (($0 as? UIAttachmentBehavior)?.items.first as? UICollectionViewLayoutAttributes)?.indexPath
        })
        return attributes.flatMap({
            return indexPaths.contains($0.indexPath) ? nil : UIAttachmentBehavior(item: $0, attachedToAnchor: $0.center)
        })
    }
...
extension UIDynamicAnimator {
    ...
    open func addBehavior(_ behavior:UIAttachmentBehavior,damping:CGFloat,frequency:CGFloat) {
