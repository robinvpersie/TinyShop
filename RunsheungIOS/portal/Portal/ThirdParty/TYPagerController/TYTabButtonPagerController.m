...
-(void)pageController:(TYTabPagerController *)pagerController clickMore:(UIButton *)sender{
    
     self.clickMore(pagerController,sender);
}
...
-(void)pagerController:(TYTabPagerController *)pagerController configreDisplayCell:(UICollectionViewCell *)cell isCurrent:(BOOL)current atIndexPath:(NSIndexPath *)indexPath{
    TYTabTitleViewCell *titleCell = (TYTabTitleViewCell *)cell;
     if (current) {
         titleCell.titleLabel.textColor = self.selectedTextColor;
         titleCell.transform =  CGAffineTransformIdentity;


    }else {
        titleCell.transform = CGAffineTransformMakeScale(self.selectFontScale, self.selectFontScale);

        titleCell.titleLabel.textColor = self.normalTextColor;
    }
