...
- (void)viewDidLoad {
    ...
    [self addlightGrayView];
    
    
}

-(void)addlightGrayView{
    UIView *lightGrayView = [[UIView alloc]initWithFrame:CGRectMake(0, _pagerBarView.frame.size.height - 0.5, _pagerBarView.frame.size.width, 0.5)];
    lightGrayView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    [_pagerBarView addSubview:lightGrayView];
}
...
- (void)addCollectionViewBar
{
    ...
    UIButton *clickMore = [UIButton buttonWithType:UIButtonTypeCustom];
    [clickMore setImage:[UIImage imageNamed:@"icon_open"] forState:UIControlStateNormal];
    clickMore.frame = CGRectMake(self.view.frame.size.width - 50, 0, 50, self.contentTopEdging);
    [clickMore addTarget:self action:@selector(clickMore:) forControlEvents:UIControlEventTouchUpInside];
    
    
    [_pagerBarView addSubview:collectionView];
    [_pagerBarView addSubview:clickMore];
...
-(void)clickMore:(UIButton *)sender {
    [self.delegate pageController:self clickMore:sender];
}
...
- (void)layoutTabPagerView
{
    ...
    _collectionViewBar.frame = CGRectMake(0, [self statusBarHeight], CGRectGetWidth(self.view.frame) - 50, self.contentTopEdging);
        _moreType.frame = CGRectMake(self.view.frame.size.width - 50, 0, 50, self.contentTopEdging);
    }else {
      _collectionViewBar.frame = CGRectMake(0, [self statusBarHeight], CGRectGetWidth(self.view.frame), self.contentTopEdging);
        _moreType.frame = CGRectMake(0, 0, 0, 0);
    }
}
