...
class ShopMchoiceView: UIButton {
    var values:NSArray?
    var data:NSArray?
    var value:UILabel?
    ...
    @objc public func getTitlesArray(titles:NSArray){
        self.values = titles
		guard self.values != nil ,self.values?.count != 0 else {
			return ;
		}
...
extension ShopMchoiceView:UITableViewDelegate,UITableViewDataSource{
...
extension ShopMchoiceView{
    ...
    @objc private func popChoiceView(){
        ...
        if self.popTableView == nil {
			...
			self.popTableView = UIImageView(frame: CGRect(x:Int( newFrame.origin.x+newFrame.size.width/2) - Int(22 + self.popWidth/2), y:Int(newFrame.maxY+5) , width: Int(55 + self.popWidth) , height: (((self.values?.count)! > 3 ? 3 : self.values?.count)!*44) + 10))
